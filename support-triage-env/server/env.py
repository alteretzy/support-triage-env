...
class SupportTriageEnv:
    ...
    def step(self, action: TriageAction) -> StepResult:
        ...
        # BUG FIX: Do NOT clamp per-step reward to [0, 1] — that kills the
        # loop penalty (-0.05).  Instead, only clamp the cumulative total.
        reward = round(reward, 4)
        self._state.cumulative_reward = round(
            min(max(self._state.cumulative_reward + reward, 0.0), 1.0), 4
        )
