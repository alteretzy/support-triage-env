...
class SupportTriageEnv:
    ...
    def step(self, action: TriageAction) -> StepResult:
        ...
        self._state.step += 1
        raw_reward, done, message = self._process_action(action)

        # Terminal actions in _process_action currently return an absolute total score.
        # Intermediate queries return step-based deltas.
        is_terminal = action.action_type in (ActionType.ROUTE, ActionType.ESCALATE, ActionType.RESOLVE)
        
        if is_terminal:
            target_score = raw_reward
        else:
            target_score = self._state.cumulative_reward + raw_reward

        # STRICTLY clamp the target cumulative score between 0.01 and 0.99
        target_score = min(max(target_score, 0.01), 0.99)
        
        # The returned step reward must be exactly the delta to reach the target_score
        # This guarantees that sum(rewards) == target_score precisely.
        step_reward = target_score - self._state.cumulative_reward
...
class SupportTriageEnv:
    ...
    def step(self, action: TriageAction) -> StepResult:
        ...
        # We want the episode's TOTAL sum of rewards to fall strictly in (0.01, 0.99).
        # We achieve this by clamping the cumulative reward directly and returning the delta.
        raw_cumulative = self._state.cumulative_reward + reward
        
        # Enforce exact bounds on the cumulative trajectory
        bounded_cumulative = min(max(raw_cumulative, 0.01), 0.99)
        # Monotonic constraint: ensure cumulative never drops, averting negative step rewards
        bounded_cumulative = max(bounded_cumulative, self._state.cumulative_reward)
        
        # The step reward is exactly what's needed to reach bounded_cumulative
        safe_reward = bounded_cumulative - self._state.cumulative_reward
        
        self._state.cumulative_reward = round(bounded_cumulative, 4)
